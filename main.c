...
// Piece Stuff
void add_piece(Piece piece, int row, int col) { board[row][col] = piece; }
void remove_piece(int row, int col) { board[row][col].type = EMPTY; }
...
// Game Stuff
int is_valid_move(Piece piece, int row, int col) { return 1; }
void move_to(int from_row, int from_col, int to_row, int to_col) {
  Piece piece = board[from_row][from_col];
  if (is_valid_move(board[from_row][from_col], to_row, to_col)) {
    board[to_row][to_col] = piece;
    board[from_row][to_col] = w_empty;
  }
}

// Board Stuff
void init_board() {
  Piece first_row[8] = {w_rook, w_knight, w_bishop, w_queen,
  ...
  Piece last_row[8] = {b_rook, b_knight, b_bishop, b_queen,
